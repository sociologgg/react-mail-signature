...
function SignIn() {
          ...
          if(errorCode =='auth/wrong-password')
            ...
            //  console.log("qwlkenlqwewq");
          ...
          else if(errorCode== 'auth/user-not-found' || errorCode == 'auth/invalid-email')
              {
                ...
                setEmailError('Kullanıcı bulunamadı. Lütfen kullanıcı adınızı kontrol edin ve tekrar deneyin.');
                setPasswordError("");
              }
  ...
  const [passwordError, setPasswordError] = useState("");
    ...
    function invalidUserError() {
      return <p class="text-16px font-roboto text-center text-error-red mt-20px">{emailError}</p>;
    }
    ...
    return (
        ...
        <div className=" relative">
          ...
          <input
           onChange={(e)=>{setPassword(e.target.value)}}
            ...
            class={`outline-none w-100%  ${emailError=="" ?(passwordError==""? `border-input focus:border-janus-focus-blue`:`border-error-red focus:border-error-red`):`border-error-red focus:border-error-red`} font-roboto text-input-gray h-10 rounded border-0.5 shadow-input p-3  mt-3`}
            placeholder="Şifre"
          />
        </div>
  
        <Link to="PassRes" class="mt-8px text-base text-janus-dark-blue text-right font-roboto">
          Şifreni mi unuttun?
        </Link>
       ...
       {loading ?<BeatLoader color={'#ffffff'} loading={true} size={10} speedMultiplier={1}/>:'Giriş Yap'}
      </button>
