...
const Dashboard = () => {
    ...
    {
        if(page == 1)
            return <Login setPage={()=>{setpage(2)}}/>
        else
            return <SignUp setPage={()=>{setpage(1)}}/>
    }
    function pageTextManger()
    {
        if(page == 1)
            return  <p class="text-2xl mt-4 font-bold font-inter text-login-red">Login</p>     
        else 
            return  <p class="text-2xl mt-4 font-bold font-inter text-login-red">SignUp</p>     
